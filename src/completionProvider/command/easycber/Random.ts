...
export class RandomCompletionProvider extends BaseCompletionProvider {
    protected commandKeyword = 'random';
    ...
    public async provideCommandCompletions(document: vscode.TextDocument, position: vscode.Position, token: vscode.CancellationToken, context: vscode.CompletionContext, commands: string[]): Promise<vscode.CompletionItem[]> {
        ...
        if (commands.length === 2) {
            return [
                this.createCompletionItem("var", "对命名空间变量进行随机赋值", "var", true, vscode.CompletionItemKind.Keyword),
                this.createCompletionItem("score", "对记分板进行随机赋值", "score", true, vscode.CompletionItemKind.Keyword),
            ];
        }

        if (commands.length === 3) {
            return [
                this.createCompletionItem("range", "范围内随机", "range", true, vscode.CompletionItemKind.Keyword),
                this.createCompletionItem("sample", "从列表中随机采样", "sample", true, vscode.CompletionItemKind.Keyword),
            ];
        }
    ...
    private handleVarRandom(document: vscode.TextDocument, position: vscode.Position, commands: string[]): vscode.CompletionItem[] {
        ...
        if (commands[2] === "range") {
            if (commands.length === 6) {
                return [this.createCompletionItem("<min>", "最小值", "", true, vscode.CompletionItemKind.Value)];
            }
            if (commands.length === 7) {
                return [this.createCompletionItem("<max>", "最大值", "", false, vscode.CompletionItemKind.Value)];
            }
            if (commands.length === 8) {
                return [
                    this.createCompletionItem("int", "整数类型", "int", false, vscode.CompletionItemKind.TypeParameter),
        ...
        if (commands[2] === "sample") {
            if (commands.length === 6) {
                return [this.createCompletionItem("<count>", "采样数量", "", false, vscode.CompletionItemKind.Value)];
            }
        }
    ...
    private handleScoreRandom(document: vscode.TextDocument, position: vscode.Position, commands: string[]): vscode.CompletionItem[] {
        if (commands[2] === "range") {
            ...
            if (commands.length === 5) {
                return this.provideSelectorCompletions(commands[4]);
            }
            if (commands.length === 6) {
                return [this.createCompletionItem("<min>", "最小值", "", true, vscode.CompletionItemKind.Value)];
            }
        ...
        if (commands[2] === "sample") {
            ...
            if (commands.length === 5) {
                return [this.createCompletionItem("<变量名>", "源列表变量名", "", true, vscode.CompletionItemKind.Variable)];
            }
            if (commands.length === 6) {
                return this.provideScoreboardCompletions(this.getWordRange(document, position, commands[5].length));
            }
            if (commands.length === 7) {
